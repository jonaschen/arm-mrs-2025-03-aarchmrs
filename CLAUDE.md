...
tools/
  ...
  build_index.py       # [TODO] One-time cache builder (run before using any skill)
  query_register.py    # [TODO] Register/field/accessor queries
  query_instruction.py # [TODO] Instruction encoding and operation queries
  query_feature.py     # [TODO] Feature/extension and version queries
  query_search.py      # [TODO] Cross-cutting keyword search
cache/                 # Generated by build_index.py — gitignored
  ...
  registers/           # One JSON per register × state (~1,800+ files)
  operations/          # One JSON per operation_id (~2,262 files)
  registers_meta.json  # Name→state index for listing and search
.claude/skills/
  arm-feat.md          # [TODO] Feature/extension queries
  arm-reg.md           # [TODO] Register field, value, and access queries
  arm-search.md        # [TODO] Cross-cutting discovery
  arm-instr.md         # [TODO] Instruction encoding and behavior queries
```
...
- **Instruction encoding is partial.** Each instruction node only carries the discriminating opcode bits (the fields that distinguish it from siblings), not the full 32-bit field breakdown. The complete encoding is assembled from the group/set hierarchy. The `arm-instr --enc` output must make this clear to avoid misinterpretation.
