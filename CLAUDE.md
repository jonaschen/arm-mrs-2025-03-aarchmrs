...
- **Phase 3 (H1–H8):** Active hardware engineering (code generation, GDB-MCP, QEMU, linter, multi-agent) — 🔲 In progress (H1–H5 complete)
...
tools/
  ...
  eval_skill.py        # [DONE] Correctness evaluation (219 tests: 51 A64 + 16 T32/A32 + 18 GIC + 24 CoreSight + 14 PMU + 14 EX + 18 H1 + 14 H3 + 23 H4 + 27 H5)
...
# Correctness evaluation (all 219 tests should pass)
...
Phase 3 builds a code-generation and verification loop on top of the spec-grounding foundation. H1–H5 are complete; H6–H8 are planned. See `AARCH64_AGENT_SKILL_DEV_PLAN.md` Part III for full detail.
...
H6, H7, and H8 are the next planned milestones. H6 depends on H1 (feature gating), H7 on H6 (security rules), H8 on H3+H4+H5 (full toolchain loop).
