...
- **Phase 3 (H1–H8):** Active hardware engineering (code generation, GDB-MCP, QEMU, linter, multi-agent) — ✅ Complete
...
tools/
  ...
  query_allowlist.py   # [DONE] Feature-qualified instruction allowlist + register blocklist (Milestone H1)
  gdb_session.py       # [DONE] GDB/MI session manager for AArch64 debugging (Milestone H3)
  query_gdb.py         # [DONE] GDB debugging CLI — step, inspect, assert, SIGILL repair (Milestone H3)
  gen_qemu_launch.py   # [DONE] QEMU launch-script generator + user-mode runner (Milestone H4)
  setup_cross_compile.py # [DONE] AArch64 cross-compilation + link strategy + 20 repair rules (Milestone H5)
  ...
  eval_skill.py        # [DONE] Correctness evaluation (387 tests: 51 A64 + 34 T32/A32 + 41 GIC + 24 CoreSight + 42 PMU + 14 EX + 18 H1 + 14 H3 + 23 H4 + 27 H5 + 42 H6 + 31 H7 + 26 H8)
...
pmu/                   # ARM PMU event data (Apache 2.0, sourced from ARM-software/data)
  cortex-a34.json      # Cortex-A34 PMU events (armv8-a, 62 events)
  cortex-a35.json      # Cortex-A35 PMU events (armv8-a, 68 events)
  cortex-a53.json      # Cortex-A53 PMU events (armv8-a, 59 events)
  cortex-a55.json      # Cortex-A55 PMU events (armv8.2-a, 111 events)
  cortex-a57.json      # Cortex-A57 PMU events (armv8-a, 84 events)
  cortex-a65.json      # Cortex-A65 PMU events (armv8-a, 129 events)
  cortex-a65ae.json    # Cortex-A65AE PMU events (armv8-a, 129 events)
  cortex-a72.json      # Cortex-A72 PMU events (armv8-a, 84 events)
  cortex-a73.json      # Cortex-A73 PMU events (armv8-a, 80 events)
  cortex-a75.json      # Cortex-A75 PMU events (armv8-a, 104 events)
  cortex-a76.json      # Cortex-A76 PMU events (armv8-a, 107 events)
  cortex-a76ae.json    # Cortex-A76AE PMU events (armv8-a, 107 events)
  cortex-a77.json      # Cortex-A77 PMU events (armv8.2-a, 107 events)
  cortex-a78.json      # Cortex-A78 PMU events (armv8.2-a, 118 events)
  cortex-a78c.json     # Cortex-A78C PMU events (armv8.2-a, 118 events)
  cortex-a510.json     # Cortex-A510 PMU events (armv9-a, 144 events)
  cortex-a520.json     # Cortex-A520 PMU events (armv9.2-a, 180 events)
  cortex-a710.json     # Cortex-A710 PMU events (armv9-a, 151 events)
  cortex-a715.json     # Cortex-A715 PMU events (armv9-a, 148 events)
  cortex-a720.json     # Cortex-A720 PMU events (armv9.2-a, 193 events)
  cortex-a725.json     # Cortex-A725 PMU events (armv9.2-a, 201 events)
  cortex-x1.json       # Cortex-X1 PMU events (armv8.2-a, 122 events)
  cortex-x1c.json      # Cortex-X1C PMU events (armv8.2-a, 118 events)
  cortex-x2.json       # Cortex-X2 PMU events (armv9-a, 151 events)
  cortex-x3.json       # Cortex-X3 PMU events (armv9-a, 155 events)
  cortex-x4.json       # Cortex-X4 PMU events (armv9.2-a, 244 events)
  cortex-x925.json     # Cortex-X925 PMU events (armv9.2-a, 244 events)
  neoverse-e1.json     # Neoverse E1 PMU events (armv8.2-a, 129 events)
  neoverse-n1.json     # Neoverse N1 PMU events (armv8.2-a, 110 events)
  neoverse-n2.json     # Neoverse N2 PMU events (armv9-a, 155 events)
  neoverse-n3.json     # Neoverse N3 PMU events (armv9.2-a, 201 events)
  neoverse-v1.json     # Neoverse V1 PMU events (armv8.4-a, 132 events)
  neoverse-v2.json     # Neoverse V2 PMU events (armv9-a, 155 events)
  neoverse-v3.json     # Neoverse V3 PMU events (armv9.2-a, 229 events)
  neoverse-v3ae.json   # Neoverse V3AE PMU events (armv9.2-a, 229 events)
  rainier.json         # Rainier PMU events (armv8.2-a, 156 events)
...
.claude/skills/
  ...
  arm-allowlist.md     # [DONE] Feature-qualified instruction allowlist (Milestone H1)
  arm-gdb.md           # [DONE] GDB-MCP debugging skill — step, inspect, assert, SIGILL repair (Milestone H3)
  arm-qemu.md          # [DONE] QEMU emulation automation — launch scripts, run, classify (Milestone H4)
  arm-cross.md         # [DONE] Cross-compilation & static linking — toolchain, -march, 20 repair rules (Milestone H5)
  arm-isa-opt.md       # [DONE] Advanced ISA optimization — SVE2/SME templates, PAC/BTI insertion, MTE helpers (Milestone H6)
  arm-linter.md        # [DONE] Linter-in-the-loop — 50 lint rules, auto-repair, lint-green gate (Milestone H7)
  arm-multi-agent.md   # [DONE] Multi-agent orchestration — Developer/Critic/Judge/Executor, RALPH loop (Milestone H8)
```
...
# Feature-qualified instruction allowlist (Milestone H1)
...
# GDB-MCP debugging (Milestone H3)
python3 tools/query_gdb.py --check                                # check gdb-multiarch
python3 tools/query_gdb.py ./my_binary --break main --registers   # run to main, show regs
python3 tools/query_gdb.py ./my_binary --step 3 --assert "x0=0"  # step + assert
python3 tools/query_gdb.py ./my_binary --suite suite.json         # run assertion suite
python3 tools/query_gdb.py --sigill-hint v9Ap4 --pc 0x4004f0     # SIGILL repair hint

# QEMU emulation (Milestone H4)
python3 tools/gen_qemu_launch.py --check                           # check QEMU install
python3 tools/gen_qemu_launch.py --mode user --cpu max             # generate user-mode script
python3 tools/gen_qemu_launch.py --mode system --cpu cortex-a57    # generate system-mode script
python3 tools/gen_qemu_launch.py --run ./my_binary --cpu cortex-a57 # run + classify
python3 tools/gen_qemu_launch.py --list-cpus                       # list AArch64 CPUs

# Cross-compilation (Milestone H5)
python3 tools/setup_cross_compile.py --check                       # check toolchain
python3 tools/setup_cross_compile.py --compile hello.c             # compile for AArch64
...
# Advanced ISA optimization (Milestone H6)
python3 tools/isa_optimize.py --list-templates                     # list all 12 templates
python3 tools/isa_optimize.py --list-templates --category sve2     # 8 SVE2 templates
python3 tools/isa_optimize.py --list-templates --category sme      # 4 SME templates
python3 tools/isa_optimize.py --template sve2-dotproduct --arch v9Ap4  # generate SVE2 code
python3 tools/isa_optimize.py --template sme-matmul --arch v9Ap2      # generate SME code
python3 tools/isa_optimize.py --auto-pac-bti --arch v9Ap0 --input func.s  # harden assembly
python3 tools/isa_optimize.py --mte-helpers --arch v8Ap5               # MTE helper header
python3 tools/isa_optimize.py --list-rules                             # 18 security rules
python3 tools/isa_optimize.py --list-rules --category pac --output json # PAC rules as JSON
python3 tools/isa_optimize.py --check-features --arch v9Ap4 SVE2 SME MTE  # feature check

# AArch64 assembly linter (Milestone H7)
python3 tools/isa_linter.py --list-rules                                   # list all 50 lint rules
python3 tools/isa_linter.py --list-rules --category security               # 18 security rules
python3 tools/isa_linter.py --list-rules --category alignment              # 8 alignment rules
python3 tools/isa_linter.py --lint test.s --arch v9Ap0                     # lint assembly file
python3 tools/isa_linter.py --lint test.s --category register --output json # filter by category
python3 tools/isa_linter.py --lint-green test.s --arch v9Ap4               # lint-green gate
python3 tools/isa_linter.py --check-vixl                                   # check VIXL availability

# Multi-agent orchestration (Milestone H8)
python3 tools/multi_agent.py --list-agents                                        # list all 4 agent roles
python3 tools/multi_agent.py --agent developer --describe                         # agent profile
python3 tools/multi_agent.py --system-prompt judge --arch v9Ap4                   # system prompt
python3 tools/multi_agent.py --lock task_1 --agent developer                      # acquire lock
python3 tools/multi_agent.py --lock-status                                        # show all locks
python3 tools/multi_agent.py --oracle --reference ref.txt --experimental exp.txt  # Oracle compare
...
python3 tools/eval_skill.py --skill multi_agent
```
...
- **PMU event descriptions are available.** Unlike the BSD AARCHMRS release, `pmu/*.json` data (Apache 2.0, from ARM-software/data) has real prose descriptions for every event. 36 CPU profiles are included, covering Cortex-A34 through Cortex-A725, Cortex-X1 through X925, Neoverse E1/N1/N2/N3/V1/V2/V3/V3AE, and Rainier.
...
Phase 3 builds a code-generation and verification loop on top of the spec-grounding foundation. H1–H7 are complete; H8 is planned. See `AARCH64_AGENT_SKILL_DEV_PLAN.md` Part III for full detail.

| Skill | Description | Status |
|-------|-------------|--------|
| H1 | AARCHMRS allowlist output (`query_allowlist.py`) | ✅ Complete |
| H2 | Hierarchical RAG on ARM ARM (DDI 0487) | 🔲 Blocked: ARM Architecture License required |
| H3 | GDB-MCP debugging (step, inspect, assert registers) | ✅ Complete |
| H4 | QEMU-AArch64 emulation automation | ✅ Complete |
| H5 | Cross-compilation & static linking (`aarch64-linux-gnu-gcc`) | ✅ Complete |
| H6 | Advanced ISA optimization (SVE2/SME/PAC/BTI/MTE, `isa_optimize.py`) | ✅ Complete |
| H7 | Linter-in-the-loop (VIXL, 50 AArch64 lint rules, `isa_linter.py`) | ✅ Complete |
| H8 | Multi-agent orchestration (Developer/Critic/Judge/Executor) | ✅ Complete |

All Phase 3 milestones (H1–H8) are complete. H8 implements multi-agent orchestration with four agent roles (Developer, Critic, Judge, Executor), a file-based task-lock protocol, Oracle comparison testing, and the RALPH continuous improvement loop.
